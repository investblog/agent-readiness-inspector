...
состав категорий снимаем калибровкой с web-UI (§4); точный живой состав (21
проверка, id и категории `discoverability/contentAccessibility/botAccessControl/
discovery/commerce`) зафиксирован в `ci/drift/snapshots/isitagentready-matrix.txt`
— единый источник, сюда не дублируем; наши `check.id` в конфиге M0 зеркалят эти
id. Дрейф ловит drift-watch CI
...
- **Главная поверхность — модель RI**: один HTML (`popup.html?sidepanel=1`) в
  двух режимах. **Chrome/Edge**: клик по иконке открывает нативную **side
  panel** (`sidePanel.setPanelBehavior({openPanelOnActionClick: true})`;
  `default_popup` вырезается манифест-хуком, иначе перехватит клик). **Firefox**:
  клик — попап (иконка не умеет открывать сайдбар), тот же HTML доступен как
  `sidebar_action`. Содержимое: скан текущей вкладки в один клик → композитный
  скор + уровень + разбивка по категориям + список проверок pass/fail/na с
  evidence. По каждому fail — кнопка «скопировать фикс-промпт» и ссылка на
  стандарт. Стиль — ближе к Lighthouse (скор + чеклист), не к DeepL. Side panel
  продуктово важна: не закрывается при работе со страницей — чинишь сайт, глядя
  на чеклист; в watch-режиме живёт открытой.
...
- **Иконка расширения — самостоятельная поверхность** (M3.5). Один бейдж, две
  роли, приоритет явный: **непрочитанные сообщения мониторинга** (`!N`, синий,
  глобальный) **> скор текущей вкладки** (цвет по зоне, per-tab) **> ничего**.
  Обратный приоритет означал бы, что при включённом авто-скане алерт не виден
  никогда. Отличие не только цветом (правило a11y): префикс `!` читается и в ч/б.
  В настройках — три переключателя: авто-скан при браузинге (**по умолчанию
  выключен**: это запросы к каждому открытому сайту), скор на иконке, счётчик
  сообщений на иконке.
...
- **инбокс сообщений** (M3.5) — алерт записывается в список **до** уведомления и
  независимо от него: разрешение `notifications` опциональное, и для всех, кто
  его не дал, инбокс и есть поверхность. Пометка прочтения — «прочитано =
  увидено»: показ списка гасит бейдж, но строки, которые были непрочитанными на
  момент рендера, остаются подсвеченными до ухода со страницы. Автопометка ждёт,
  пока страница действительно перед глазами (видимость **и** фокус окна).
...
- **Модель permissions — путь Redirect Inspector** (минимальные скоупы, ничего
  ...
  - копирование фикс-промптов — `navigator.clipboard` из popup по жесту, без
    `clipboardWrite`;
  - `sidePanel` (Chromium) — с M1, вместе с главной поверхностью (безобидное
    разрешение без install-warning, как в RI);
...
- ~~Монетизация: полностью бесплатно + новостной канал, или pro-тир (мониторинг /
  fix-apply) поверх бесплатного сканера.~~ **Решено (2026-07-31): вопрос снят —
  платных тиров нет.** Расширение полностью бесплатно; «монетизация» в портфеле
  301.st = owned-канал: промо-блок сервиса внутри UI (паттерн RI: логотип +
  что такое 301.st + CTA с utm), ссылки в футерах, uninstall-фидбек, кросс-промо
  и новости (M5). Никаких pro-фич, лимитов и платных стен.
