...
**Полностью переведён интерфейс на `en` и `ru`.** Для `de`, `es`, `pt-BR`, `fr`
в пакете лежат **разрежённые локали** — только `extName` и `extDescription`;
остальные 136 строк падают в английский по штатному фолбэку.
